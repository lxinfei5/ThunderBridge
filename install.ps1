<#
.SYNOPSIS
  ThunderBridge installer for Windows.
...
$RepoUrl = "https://github.com/lxinfei5/ThunderBridge.git"
...
if (-not $Dir)    { $Dir    = if ($env:UC_INSTALL_DIR) { $env:UC_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA "ThunderBridge" } }
if (-not $BinDir) {
...
if (-not (Test-Repo $Repo)) { Die "internal: '$Repo' is not a valid ThunderBridge checkout" }
...
$shimBody = @(
    '@echo off',
    'REM ThunderBridge launcher (generated by install.ps1). Points at your checkout.',
    "call `"$RepoCmd`" %*"
) -join "`r`n"
...
Write-Host ""
Info "Done. ThunderBridge is installed."
Write-Host ""
if ($onPath) {
