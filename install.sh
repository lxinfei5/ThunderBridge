#!/usr/bin/env bash
# ThunderBridge installer (macOS / Linux / WSL).
#
...
#                    (default: $UC_INSTALL_DIR or ~/.thunderbridge)
...
REPO_URL="https://github.com/lxinfei5/ThunderBridge.git"
INSTALL_DIR="${UC_INSTALL_DIR:-$HOME/.thunderbridge}"
...
is_repo() { [ -f "$1/proxy.py" ] && [ -f "$1/bin/ultracode" ] && [ -f "$1/bin/thunderbridge" ]; }
...
is_repo "$REPO" || die "internal: '$REPO' is not a valid ThunderBridge checkout"
...
cat > "$SHIM" <<EOF
#!/usr/bin/env bash
# ThunderBridge launcher (generated by install.sh). Points at your checkout;
...
say ""
info "${B}Done.${X} ThunderBridge is installed."
say ""
if [ "$ON_PATH" = "1" ]; then
